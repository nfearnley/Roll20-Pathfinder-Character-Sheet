...
function importItems(items, resources, armorPenalties, armor, weapons) {
    ...
    getSectionIDs(repeatPrefix, function(idarray) {
        var itemNameAttrs = _.union(_.map(idarray, function(id) {
        ...
        getAttrs(itemNameAttrs, function(names) {
            ...
            _.each(items, function(item) {
                ...
                attrs[repeatPrefix + "_value"] = parseFloat(item.cost._value) / parseInt(item._quantity, 10);
                ...
                if (item._name.toLowerCase().indexOf("shield") !== -1) {
                    ...
                    weaponCompareName = _.find(weaponNames, function(name) {
                        return name.toLowerCase().indexOf(attackName) !== -1;
                    }) || item._name;
                }
                ...
                if (item._name.toLowerCase().indexOf("klar") !== -1) {
                    armorCompareName = _.find(armorNames, function(name) {
                        return name.toLowerCase().indexOf("klar") !== -1;
                    }) || item._name;
                }
...
function importClasses(attrs, classes, hitdice) {
    var classList = new Object();
    var diceList = _.without(_.map(hitdice.split(";")[hitdice.split(";").length - 1].trim().split(/[+-]/), function(hitdie) {
        ...
        if (classIndex !== -1) {
            ...
            if (abMod !== 0) {
                ...
                if (!_.isUndefined(spellclasses[i].spelllevel)) {
                    ...
                    for (j = 0; j < 10; j++) {
                        attrs["spellclass-" + spellClassIndex + "-level-" + j + "-spells-per-day_max"] = 0;
                    }
...
function importSpells(spells, spellclasses) {
    ...
    getSectionIDs(repeatPrefix, function(idarray) {
        var spellNameAttrs = _.union(_.map(idarray, function(id) {
            return repeatPrefix + "_" + id + "_name";
...
function calcHitDice(hitdice) {
    ...
    var i = 0;
    while (i < dice.length) {
        numDice += parseInt(dice[i].split("d")[0], 10);
        i++;
    }
    return numDice;
}
...
function importSkills(attrs, skills, size, ACP) {
    ...
    for (i = 0; i < skills.length; i++) {
        ...
        if (skill._classskill === "yes") {
            attrs[skillAttrPrefix + "-cs"] = 3;
        }
        ...
        if (skill._trainedonly === "yes") {
            attrs[skillAttrPrefix + "-ReqTrain"] = 1;
        }
...
function importPenalties(attrs, penalties) {
    ...
    while (i < penalties.length) {
        if (penalties[i]._name === "Armor Check Penalty") {
            ...
            attrs["armor3-max-dex"] = Math.min(99, parseNum(penalties[i]._value));
        }    // Hero Lab uses 1000 for Max Dex when player doesn't have one; cap it at 99 to match sheet default
        i++;
    }
    return ACP;
}
...
function importCharacter(characterObj) {
    ...
    if (!_.isUndefined(characterObj.classes.class)) {
        ...
        if (!_.isUndefined(characterObj.spellclasses.spellclass)) {
            ...
            spellsArray = _.reject(spellsArray, function(spell) {
                if (_.contains(spellNames, spell._name)) {
                    return true;
                }
    ...
    var weapons = _.reject(arrayify(characterObj.melee.weapon || {}).concat(arrayify(characterObj.ranged.weapon || {})), function(item) {
        return _.isUndefined(item._name);
    });

    // "Tracked Resources" is a list of uses, either a quantity of items, charges, or uses per day
    var resources = _.object(_.map(characterObj.trackedresources.trackedresource, function(resource) {
        return [resource._name, resource];
    }));

    // Make an array of items, both magic and mundane
    var items = _.reject(arrayify(characterObj.magicitems.item || {}).concat(arrayify(characterObj.gear.item || {})), function(item) {
        return _.isUndefined(item._name);
    });
    ...
    var specials = _.reject(arrayify(characterObj.attack.special).concat(arrayify(characterObj.defenses.special), arrayify(characterObj.otherspecials.special), arrayify(characterObj.movement.special), arrayify(characterObj.defensive.special)), function(obj) {
    ...
    getSectionIDs("repeating_ability", function(idarray) {
        var abilityNameAttrs = _.union(_.map(idarray, function(id) {
        ...
        getAttrs(abilityNameAttrs, function(abilityAttrs) {
            ...
            var featuresArray = _.filter(abilityObjList, function(obj) {
                return !(obj.rulecategory === "traits" || obj.rulecategory === "feats");
            });
            var featuresList = {};
