...
var attkpenaltySubtractFromFields = ["condition-Dazzled", "condition-Entangled", "condition-Grappled", "condition-Fear", "condition-Sickened", "woundsPenaltyTotal"];
var attkpenaltySumRow = ["attk-penalty"].concat(attkpenaltyAddToFields);
var groupMapForMenu = { "0": "none", "@{attk-melee}": "melee", "@{attk-melee2}": "melee",
...
function updateAttack(attype, v, setter) {
    var currtot = 0;
    var newtot = 0;
    try {
        setter = setter || {};
...
function getOneSetAttackFields(attype) {
    ...
    if (attackGridFields[attype]) {
        fields = _.chain(attackGridFields[attype]).values().flatten().compact().uniq().reject(function(a) {
            return a.indexOf("macro") >= 0;
        }).value().sort();
    }
...
function getAttackFields(attypes) {
    ...
    fields = _.chain(validtypes).map(function(key) {
        return _.values(attackGridFields[key]);
    }).flatten().compact().uniq().reject(function(a) {
        return a.indexOf("macro") >= 0;
    }).value().sort();
    fields.push("attk-penalty");
    fields.push("buff_attack-total");
 ...
 */
function updateAttackAsync(attype, callback, silently) {
    var done = _.once(function() {
        if (typeof callback === "function") {
            callback();
        }
...
export function recalculate(callback, silently, oldversion) {
    ...
    var callUpdateAttacksAndDamage = _.once(function() {
...
function getWoundsTotal(v) {
    getAttrs(["condition-Wounds", "wounds_gritty_mode", "has_endurance_feat", "wound_threshold-show", "woundsPenaltyTotal"], function(v) {
        ...
        var wounds = (parseInt(v["wound_threshold-show"], 10) || 0) * woundPenalty;
        setAttrs({
