...
var totColumns = _.values(buffToTot).concat(["dodge"]).sort();
...
var buffTotFields = _.chain(totColumns).map(function(totstr) {
...
var charHelperFields = {
    "armor": ["armor3-equipped"],
    "shield": ["shield3-equipped"]
};
// reverse otherCharBonuses: fields on sheet that affect buffs (all leaf nodes of otherCharBonuses)
var charBonusFields = _.chain(otherCharBonuses).values().map(function(v) {
    return _.values(v);
}).flatten().uniq().value().sort();
// note fields
var buffNoteFields = ["buff_attack_notes", "buff_save_notes", "buff_init_notes", "buff_skill_notes", "buff_defense_notes"];
...
var bonusesWithNoTypes = ["size", "hptemp"];
// all attributes on a buff row, to make getAttrs easier when totalling
var buffRowAttrs = ["_b1-show", "_b1_val", "_b1_bonus", "_b1_bonustype",
...
function getBuffNotes(id, v, setter) {
    ...
    var prefix = "repeating_buff2_" + idStr;
    var notestr = "";
    var buffstr = "";
    var notefield = "";
    var notereg;
...
export function clearBuffTotals(callback, silently) {
    ...
    getAttrs(buffTotFields, function(v) {
        ...
        setter = _.reduce(totColumns, function(memo, col) {
            ...
            var exists = parseInt(v["buff_" + col + "_exists"], 10) || 0;
            if (val ) {
        ...
        setter = _.reduce(PFAbilityScores.abilities, function(memo, col) {
            ...
            var exists = parseInt(v["buff_" + col + "_penalty_exists"], 10) || 0;
            if (val ) {
...
function assembleRows(ids, v, col) {
    ...
    var rows = ids.reduce(function(m, id) {
        var valArray;
        var prefix = "repeating_buff2_" + id + "_";
        try {
            valArray = buffsPerRow.reduce(function(im, n) {
                var innerPrefix = prefix + n;
                var bonusField = innerPrefix + "_bonus";
                var vals = {};
                try {
...
function updateBuffTotalAsync(col, callback, silently) {
    ...
    var isAbility = (PFAbilityScores.abilities.indexOf(col) >= 0) && col.indexOf("skill") < 0;
    ...
    getSectionIDs("repeating_buff2", function(ids) {
        ...
        if (ids && _.size(ids)) {
            try {
                ...
                totfields = totals.map(function(t) {
                    return "buff_" + t + "-total";
                }).concat(totals.map(function(t) {
...
function updateAllBuffTotalsAsync(callback, silently, eventInfo) {
    ...
    getSectionIDs("repeating_buff2", function(ids) {
        ...
        fields = fields.concat(["armor3-equipped", "shield3-equipped"]);
...
function reEvaluateCustomMacros(callback, silently) {
    ...
    var buffRowMacros2 = ["_b1-show", "_b1_val", "_b1_macro-text",
        ...
        "_enable_toggle"];
    function recalculateBuffRow(callback, id, v) {
        ...
        try {
            if (_.size(ids) > 0) {
                ...
                getAttrs(fields, function(v) {
                    ...
                    var doneRow = _.after(numRows, done);
                    ids.forEach(function(id) {
 ...
 * @param {Map} eventInfo
 */
function addCommonBuff(callback, eventInfo) {
    var setter = {};
    var fields;
