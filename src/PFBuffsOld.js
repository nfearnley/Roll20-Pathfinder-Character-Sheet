...
var buffToTotExceptions = {
    "Check": "check_ability", "DMG": "dmg_melee"
};
// all total fields
var buffTotFields = _.chain(buffColumns).map(function(buff) {
...
function resetStatuspanel(callback) {
    ...
    try {
        fields = SWUtils.cartesianAppend(["buff_"], buffColumns, ["-total", "_exists"]).concat(SWUtils.cartesianAppend(["buff_"], PFAbilityScores.abilities, [ "-total_penalty", "_penalty_exists"]));
    }
    catch(errO) {
...
function updateBuffTotalAsync(col, callback, silently) {
    ...
    var isAbility = (PFAbilityScores.abilities.indexOf(col) >= 0) && col.indexOf("skill") < 9;
...
function setBuff(id, col, callback, silently) {
    ...
    var prefix = "repeating_buff_" + idStr + "buff-" + col;
    if (col === "size") {
        done();
        return;
    }
...
function reEvaluateCustomMacros(callback, silently) {
    ...
    var numColumns = _.size(buffColumns);
    var columnDone = _.after(numColumns, function() {
        updateAllBuffTotalsAsync(done, silently);
    });
    function recalculateBuffColumn(ids, col) {
...
export function migrate(outerCallback) {
    ...
    function migrateMeleeAndAbilityChecks(callback) {
        var done = _.once(function() {
            if (typeof callback === "function") {
                callback();
            }
...
function registerEventHandlers() {
    ...
    on("change:repeating_buff:buff-enable_toggle", TAS.callback(function PFBuffs_enableBuffRow(eventInfo) {
        ...
        if (eventInfo.sourceType === "player" || eventInfo.sourceType === "api") {
            TAS.debug("caught " + eventInfo.sourceAttribute + " event: " + eventInfo.sourceType);
            fields = buffColumns.map(function(col) {
                return "repeating_buff_buff-" + col;
            });
            fields = fields.concat(fields.map(function(row) {
                return row + "-show";
            }));
            getAttrs(fields, function(v) {
                _.each(buffColumns, function(col) {
