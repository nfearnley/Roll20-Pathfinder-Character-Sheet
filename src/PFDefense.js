...
export var defenseArmorShieldRows = ["armor3", "shield3"];
export var defenseArmorShieldColumns = ["equipped", "acbonus", "enhance", "max-dex", "acp", "spell-fail", "proficiency", "type"];
var defenseFieldTotals = ["acp", "max-dex", "AC-armor", "AC-shield", "spell-fail", "acp-attack-mod", "max-dex-source", "current-load"];
var defenseArmorFields = SWUtils.cartesianAppend(defenseArmorShieldRows, ["-"], defenseArmorShieldColumns).concat(defenseFieldTotals);
var events = {
...
export function updateDefenses( callback, silently, eventInfo) {
    ...
    getAttrs(["AC-ability-mod", "FF-DEX", "AC-penalty", "CMD-penalty", "size", "max-dex", "AC-dodge",
        ...
        var dodge = parseInt(v["AC-dodge"], 10) || 0;
        var deflect = parseInt(v["AC-deflect"], 10) || 0;
        var miscAC = parseInt(v["AC-misc-mod"], 10) || 0;
        var condPenalty = parseInt(v["AC-penalty"], 10) || 0;
        var buffs = parseInt(v["buff_AC-total"], 10) || 0;
        var buffsTouchOnly = parseInt(v["buff_Touch-total"], 10) || 0;
        var buffsCMDOnly = parseInt(v["buff_CMD-total"], 10) || 0;
        //     buffsFFcmdOnlyDropDodge = parseInt(v["buff_ffCMD-nododge"], 10) || 0,
        var armor = parseInt(v["AC-armor"], 10) || 0;
        var shield = parseInt(v["AC-shield"], 10) || 0;
        var natural = parseInt(v["AC-natural"], 10) || 0;
        var bab = parseInt(v.bab, 10) || 0;
        var miscCMD = parseInt(v["CMD-misc-mod"], 10) || 0;
        var maxDex = parseInt(v["max-dex"], 10);
        var cmdPenalty = parseInt(v["CMD-penalty"], 10) || 0;
        var blinded = parseInt(v["condition-Blinded"], 10) || 0 ? 1 : 0;
        var pinned = parseInt(v["condition-Pinned"], 10) || 0 ? 1 : 0;
        var stunned = parseInt(v["condition-Stunned"], 10) || 0 ? 1 : 0;
        var ffed = parseInt(v["condition-Flat-Footed"], 10) || 0 ? 1 : 0;
        var cowering = parseInt(v["condition-Cowering"], 10) || 0 ? 1 : 0;
        var maxDexSource = parseInt(v["max-dex-source"], 10) || 0;
        var currload = parseInt(v["current-load"], 10) || 0;
        var dexModShowLimit = 0;
        var currDexModLimit = parseInt(v["maxdex-toggle"], 10) || 0;
        var noDexShowLimit = 0;
        var currNoDexLimit = parseInt(v["nodex-toggle"], 10) || 0;
        var unlockDefAbility = parseInt(v.unlock_def_ability, 10) || 0;
        var lockDefAbility = unlockDefAbility ? 0 : 1;
        var ac = 10;
        var touch = 10;
        var ff = 10;
        var cmd = 10;
        var cmdFF = 10;
        var currAC = parseInt(v.AC, 10);
        var currTouch = parseInt(v.Touch, 10);
        var currFF = parseInt(v["Flat-Footed"], 10);
        var currCMD = parseInt(v.CMD, 10);
        var currCMDFF = parseInt(v["FF-CMD"], 10);
        var currUncanny = parseInt(v.uncanny_dodge, 10) || 0;
        var acAbilityName = PFUtils.findAbilityInString(v["AC-ability"]);
        var uncannyAbilityName = currUncanny ? acAbilityName : PFUtils.findAbilityInString(v["FF-ability"]);
        var uncannyCMDabilityName = lockDefAbility ? uncannyAbilityName : PFUtils.findAbilityInString(v["CMD-ability"]);
        var cmdAbilityDDvalName = lockDefAbility ? acAbilityName : PFUtils.findAbilityInString(v["CMD-ability2"]);
        var ability = parseInt(v["AC-ability-mod"], 10) || 0;
        var cmdAbility1 = parseInt(v["CMD-STR"], 10) || 0;
        var cmdAbility2 = lockDefAbility ? ability : parseInt(v["CMD-DEX"], 10) || 0;
        var ffAbility = 0;
        var cmdFFAbility2 = 0;
        var loseDex = 0;
        var immobilized = 0;
        var armorbuff = parseInt(v["buff_armor-total"], 10) || 0;
        var shieldbuff = parseInt(v["buff_shield-total"], 10) || 0;
        var naturalbuff = parseInt(v["buff_natural-total"], 10) || 0;
        var buffFFOnly = parseInt(v["buff_flat-footed-total"], 10) || 0;
        var dodgebuff = parseInt(v["buff_dodge-total"], 10) || 0;
        var buffac = 0;
        var ffdodge = 0;
        var ffdodgebuff = 0;
        var bufftouch = 0;
...
function setDefenseDropdownMod(dropdownField, callback, silently, eventInfo, doNotCallUpdateDefenseAfter) {
    ...
    var updateAndDone = _.once(function() {
...
function updateArmor(callback, silently, eventInfo) {
    ...
    getAttrs(defenseArmorFields, function(v) {
        ...
        var currSpellFail = 0;
        var currAtkMod = 0;
        var encumbranceDD = parseInt(v["max-dex-source"], 10) || 0;
        var currentLoad = parseInt(v["current-load"], 10) || 0;
        var setter = {};
        var params = {};
        try {
...
export function recalculate(callback, silently, oldversion) {
    ...
    var numDropdowns = _.size(defenseDropdowns);
    var doneOneDefenseDropdown = _.after(numDropdowns, function() {
