...
/**
 * Sets wound thresholds by looking up values for "are we even using wound threshold rules?" and the max hit points.
...
/**
 * Looks up data and calls updateCurrHP
 */
...
export function updateMaxHPLookup(callback, silently, eventInfo, forceReset) {
    ...
    function(v) {
        ...
        var tempHP = 0;
        var newHP = 0;
        var increaseHPWhenMaxHPIncreases = 0;
        var nonLethal = 0;
        var newHPMax = 0;
        var mythic = 0;
        var currWoundLevel = 0;
        var usesWounds = 0;
        var setter = {};
        try {
...
function ensureNPCHPZero(callback) {
    getAttrs(["npc-hd", "npc-hd-num", "NPC-HP", "is_npc"], function(v) {
        ...
        var npcLevels = parseInt(v["npc-hd-num"], 10) || 0;
        var npcHP = parseInt(v["NPC-HP"], 10) || 0;
...
export function recalculate(callback, silently, oldversion) {
    ...
    var resetWounds = _.once(function() {
        setWoundThreshholdsLookup();
        done();
    });
    var callUpdateMaxHPLookup = _.once(function() {
        updateMaxHPLookup(resetWounds, silently);
    });
    var callUpdateTempHP = _.once(function() {
