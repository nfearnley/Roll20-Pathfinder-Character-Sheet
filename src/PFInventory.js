...
var wornSlotTypes = locationNamesOld.concat(["Armor3", "Shield3"]);
var wornSlotsRolls = _.map(wornSlotTypes, function(row) {
    return "worn-" + row + "-roll";
});
var locationNames = ["Carried", "NotCarried", "Armor", "Belt", "Body", "Chest", "Eyes", "Feet", "Hands", "Head", "Headband", "Neck", "Ring1", "Ring2", "Shield", "Shoulders", "Wrist"];
var locationMap = { "Carried": 0, "NotCarried": 1, "Armor": 2, "Belt": 3, "Body": 4, "Chest": 5, "Eyes": 6, "Feet": 7, "Hands": 8,
    "Head": 9, "Headband": 10, "Neck": 11, "Ring1": 12, "Ring2": 13, "Shield": 14, "Shoulders": 15, "Wrist": 16,
    "equipped": 2 };
var equipMap = { "noEquipType": 0, "Weapon": 1, "Armor": 2, "Ammo": 3, "Consumables": 4, "OtherMagic": 5, "Gear": 6, "Other": 7, "Charged": 8, "Other2": 9 };
var groupMapForMenu = { 0: "", 1: "weapons", 2: "armor-shield", 3: "ammunition", 4: "consumables", 5: "other-magic-items", 6: "gear-tool", 7: "other-items", 8: "charged-magics", 9: "other-items-2" };
var totaledFields = { "value": 1, "hp": 1, "weight": 1 };
var commonLinkedAttributes = ["attack-type", "range", "masterwork", "crit-target", "crit-multiplier", "damage-dice-num", "damage-die", "damage",
...
function migrateWornEquipment(callback) {
    ...
    var wornEquipmentColumns = ["charges", "weight", "hp", "hp_max", "value"];
    function copyWornEquipmentToNewItem( row, callback) {
        var done = _.once(function() {
        ...
        getAttrs(attribList, function(v) {
            ...
            var attrib = "";
            var newLocation = 0;
            var newEquipType = equipMap.noEquipType;
    ...
    function copyWornDefenseToNewItem(item, wornAlreadySet, callback) {
    ...
    getAttrs(["migrated_worn_equipment"], function(v) {
        ...
        var foundWornShield = false;
        var doneAll = _.after(2, function() {

            doneMigrating();
        });
        function migrateDefenses() {
        ...
        var doneWornRows = _.after(_.size(locationNamesOld), function() {
            var wornRows;
            var fieldsToClear;
            var setter;
            try {
                ...
                setter = _.reduce(fieldsToClear, function(m, f) {
                    m[f] = "";
                    return m;
                }, {});
...
function updateEquipmentLocation(id, callback, silently, eventInfo) {
    ...
    var realItemID = id || (eventInfo ? SWUtils.getRowId(eventInfo.sourceAttribute) || "" : "");
    var prefix = "repeating_item_" + realItemID + "_";
    var locationField = prefix + "location";
    var locationTabField = prefix + "loctype-tab";
    var nameField = prefix + "name";
    var oldLocationField = prefix + "old_location";
    var rollField = prefix + "macro-text"
    ;
    try {
        ...
        getAttrs([locationField, oldLocationField, nameField, locationTabField], function(v) {
            ...
            try {
                ...
                loctab = parseInt(v[locationTabField], 10);
                if (isNaN(loctab)) {
                    loctab = -2;
                }
...
function updateWornArmorAndShield(location, sourceAttribute, callback) {
    ...
    var wornSlotField = "";
    var item_entry = "";
    var itemFields = ["item-acbonus", "item-acenhance", "item-max-dex", "item-acp", "item-spell-fail", "item-defense-type", "item-proficiency",
            ...
            getAttrs(fields, function(w) {
                ...
                try {
                    ...
                    _.each(idarray, function(currentID) { // loop through the set
                        var targetVal = "";
                        var wField = ""; // start with blank in this loop
...
function getEquipmentTypeFromName(name) {
    var tempstr;
    var currType = equipMap.noEquipType;
    var matches;
    if (!name) {
        return currType;
    }
...
function importFromCompendium(eventInfo) {
    ...
    var fields = ["default_char_size", "equipment_tab",
...
export function recalculate(callback, silently, oldversion) {
    ...
    var setTotals = _.after(2, function() {
    ...
    on("change:repeating_item:location", TAS.callback(function eventUpdateItemLocation(eventInfo) {
        ...
        if (eventInfo.sourceType === "player" || eventInfo.sourceType === "api") {
            getAttrs(["repeating_item_location", "repeating_item_old_location"], function(v) {
                ...
                var oldLoc = parseInt(v.repeating_item_old_location, 10);
    ...
    on("change:repeating_item:equip-type", TAS.callback(function eventItemEquipTypeChange(eventInfo) {
        ...
        if (eventInfo.sourceType === "player" || eventInfo.sourceType === "api") {
            getAttrs(["repeating_item_equip-type", "repeating_item_equiptype-tab", "equipment_tab"], function(v) {
                ...
                var oldtype = parseInt(v["repeating_item_equiptype-tab"], 10);
                var equipTab = parseInt(v.equipment_tab, 10) || 0;
                var setter = {};
