...
function getRepeatingCommandMacro(baseAttribs, callback, header) {
    function done(macro, origMacro) {
        if (typeof callback === "function") {
            callback(macro, origMacro);
        }
    }
    var defaultTemplate = "pf_block";
    var defaultHeader = "header_image-pf_block";
    var defaultName = "ability-menus";
    var nameField = "name";
    var bonusField = "";
    var usesField = "";
    var altUsesField = "";
    ...
    var noRows = " {{description=^{none-available} }}";
    ...
    getSectionIDs("repeating_" + baseAttribs.section, function(ids) {
        var fields = [];
        var prefix = "repeating_" + baseAttribs.section + "_";
        try {
            if (!ids || _.size(ids) <= 0) {
