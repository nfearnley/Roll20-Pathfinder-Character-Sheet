...
/**
 * Saves feats that require updates to the sheet in an object, no spaces and all lowercase.
...
/**
 * Parses statblock hp string such as 203 (14d10+126)
...
/**
 *
 * Parses AC string from statblock
    ...
    try {
        ...
        if (matches && matches[1]) {
            ...
            if (matches && matches[1] && matches[2]) {
                acMap.dex = parseInt(matches[1], 10) || 0;
                // sshould not happen anymore since 6th printing of PRD they removed abilities that change ability to AC, now
...
/**
 * Returns object with speeds {land:base,fly:xx,swim:xx} etc
 */
...
/**
 * Get names of an attack or special attack
...
/**
 * Parse attack string one at a time, returns arrays grouped by full attacks
...
function parseAttacks(atkstr, atktypestr, cmbval) {
    ...
    try {
        ...
        attacksouter = _.reduce(atkarrayout, function(memoout, atkstrout, groupidx) {
            ...
            var attacks;
            try {
                atkarray = SWUtils.splitByCommaIgnoreParens(atkstrout);
...
function parseSkillRacialBonuses(racialstr) {
    ...
    var allCoreSkillsLower = _.map(PFSkills.allCoreSkills, function(skill) {
        return skill.toLowerCase();
    });
    var skillsWithSubSkillsLower = _.map(PFSkills.skillsWithSubSkills, function(skill) {
        return skill.toLowerCase();
    });
    var skillsWithSpaces = PFSkills.skillsWithSpaces;   // PFSkills.skillsWithSpaces doesn't exist
    var temparray;
    var modifiers = [];
    var abilitymodstr = "";
    var abilitymodlower = "";
    var ability = "";
    var setability = false;
    var tempskill = "";
    var skillmods = {};
    var skillnotes = [];
    var abilitymods = {};
...
function parseSkills(skillstr) {
    ...
    var skills = _.reduce(rawSkills, function(memo, skill) {
...
function parseSpecialAttacks(setter, saString, cmb) {
    ...
    retarray = SWUtils.splitByCommaIgnoreParens(saString);
...
/**
 * parses spell string from compendium and returns js object
...
function createSpellEntries(spellObj, casterObj, setter) {
    ...
    _.each(spellObj.spellsByLevel, function(spellLevel) {
        ...
        setter = _.reduce(spellLevel.spells, function(memo, spell) {
            ...
            var thisDC = 0;
            var prefix = "repeating_" + section + "_" + newRowId + "_";
            try {
...
/**
 * creates rows in repeating_weapon
...
function createACEntries(acMap, abilityScores, importantFeats, hpMap, bab, level, setter) {
    var acAbility = "DEX";
    var acDexDef = abilityScores.dex.mod;
    var calcCMD = 0;
    var altbab = 0;
    try {
        setter = setter || {};
...
function createSkillEntries(skills, racial, abilityScores, importantFeats, classSkills, sizeMap, isUndead, setter) {
    var tempAbilities = PFSkills.coreSkillAbilityDefaults;  // PFSkills.coreSkillAbilityDefault doesn't exist
    var tempstr = "";
    try {
        setter = setter || {};
        ...
        if (racial) {
            if (racial.abilitymods && _.size(racial.abilitymods) > 0) {
                ...
                tempAbilities = _.extend({}, PFSkills.coreSkillAbilityDefaults, racial.abilitymods);    // PFSkills.coreSkillAbilityDefault doesn't exist
            }
...
/**
 * Adds init,init-misc,init-misc-mod,init-ability-mod values to setter
 * @param {Map<string,any>} setter the map to pass to SWUtils.setWrapper
...
function createFeatEntries(featlist, race, level, setter) {
    return _.reduce(featlist, function(memo, feat) {
        var newRowId = generateRowID();
        var prefix = "repeating_ability_" + newRowId + "_";
...
function importFromCompendium(eventInfo, callback, errorCallback) {
    ...
    var errorDone = _.once(function() {
        TAS.warn("Leaving importFromCompendium NOTHING DONE");
        if (typeof errorCallback === "function") {
            errorCallback();
        }
    });
    var fields = npcCompendiumAttributesPlayer.concat(["is_npc", "alignment", "npc_parse_no_recalc"]);
    getAttrs(fields, function(v) {
        ...
        var allSoFar = {};
        var specialQualities = [];
        var recalcWhenDone = 0;
        var baseSaves = {};
