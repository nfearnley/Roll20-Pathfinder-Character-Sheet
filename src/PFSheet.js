...
function setupNewSheet(callback) {
    var done = _.once(function() {
        var setter = {
            ...
            "is_v1": 1,
            "use_buff_bonuses": 1,
            "use_advanced_options": 0,
            "modify_dmg_by_size": 1,
            "PFSheet_Version": String(PFConst.version.toFixed(2))
        };
    ...
    getAttrs(["is_npc", "set_pfs"], function(v) {
        ...
        var isPFS = parseInt(v.set_pfs, 10) || 0;
...
function recalculateParallelModules(callback, silently, oldversion) {
    ...
    var numberModules = _.size(parallelRecalcFuncs);
    var doneOneModuleInner = _.after(numberModules, done);
    var curr = 0;

    function doneOneModule() {
        curr++;
...
function recalculateDefenseAndEncumbrance(callback, silently, oldversion) {
    ...
    function callEncumbrance() {
        PFEncumbrance.recalculate(done, silently, oldversion);
    }
    var doneBeforeEncumbrance = _.after(2, callEncumbrance);
    try {
...
export function recalculate(oldversion, callback, silently) {
    ...
    var callParallel = TAS.callback(function callRecalculateParallelModules() {
        recalculateParallelModules(done, silently, oldversion);
    });
    var callEncumbrance = TAS.callback(function callRecalculateDefenseAndEncumbrance() {
        recalculateDefenseAndEncumbrance(callParallel, silently, oldversion);
    });
    var callRecalcCore = TAS.callback(function callRecalculateCore() {
...
/**
 *  Looks at current version of page in PFSheet_Version and compares to code PFConst.version
...
export function checkForUpdate(forceRecalc) {
    ...
    getAttrs(["PFSheet_Version", "migrate1", "recalc1", "is_newsheet", "is_v1", "hp", "hp_max", "npc-hd", "npc-hd-num",
        "race", "class-0-name", "npc-type", "level", "hide_announcements", PFConst.announcementVersionAttr
    ], function(v) {
        var setter = {};
        var migrateSheet = false;
        var newSheet = false;
        var recalc = false;
        var currVer = parseFloat(v.PFSheet_Version, 10) || 0;
        function setUpgradeFinished() {
            SWUtils.setWrapper({
                recalc1: 0,
                migrate1: 0,
                is_newsheet: 0,
                character_sheet: "Pathinder_Neceros v" + String(PFConst.version),
                PFSheet_Version: String(PFConst.version.toFixed(3))
    ...
    on("change:repeating_weapon:source-item", TAS.callback(function eventUpdateAttackSourceItem(eventInfo) {
        if (eventInfo.sourceType === "player" || eventInfo.sourceType === "api") {
            ...
            getAttrs([eventInfo.sourceAttribute], function(v) {
                ...
                var sourceId = v[eventInfo.sourceAttribute];
