...
function updateCasterRanges(spellclassidx, eventInfo, force, callback, silently) {
    ...
    var lvlField = prefix + "-level-total";
    var closeField = prefix + "-close";
    var medField = prefix + "-medium";
    var longField = prefix + "-long";
    getAttrs([lvlField, closeField, medField, longField], function(v) {
        var level = parseInt(v[lvlField], 10) || 0;
        var closeRng = parseInt(v[closeField], 10) || 0;
        var medRng = parseInt(v[medField], 10) || 0;
        var longRng = parseInt(v[longField], 10) || 0;
        var ranges = {};
        var setter = {};
        var params = {};
        try {
...
function updateSaveDCs(classidx, eventInfo, callback, silently) {
    ...
    getAttrs(["use_spells", "spellclass-" + classidx + "-exists", "Concentration-" + classidx + "-mod", "spellclass-" + classidx + "-level-0-savedc"], function(v) {
        ...
        var dcLvlZero = 10 + mod;
        var currDC = parseInt(v["spellclass-" + classidx + "-level-0-savedc"], 10);
        var setter = {};
        var params = {};
        var i;
        try {
...
function updateBonusSpells(classidx, eventInfo, callback, silently) {
    ...
    getAttrs([conAbility, "INT", "WIS", "CHA", "STR", "DEX", "CON",
        ...
        var abilityVal = parseInt(v[abilityName], 10);
        var setter = {};
        var params = {
            silent: true
        };
        ...
        var bonusName;
        var i;
        var prefix = "spellclass-" + classidx + "-level-";
        try {
            if (!isNaN(abilityVal)) {
                if (abilityVal >= 12) {
                    for (i = 1; i < 10; i++) {
...
function recalcOneClass(spellClassIdx, callback, silently) {
    ...
    var doneOne = _.after(5, done);
    var doneOneLevel = _.after(10, doneOne);
...
function updateCasterLevel(spellclassidx, eventInfo, classlevel, callback, silently, forceRecalc) {
    ...
    getAttrs(["use_spells", "spellclass-" + spellclassidx + "-level", "spellclass-" + spellclassidx + "-level-total", "spellclass-" + spellclassidx + "-level-misc-mod", "buff_CasterLevel-total", "CasterLevel-Penalty", "spellclass-" + spellclassidx + "-exists"], function(v) {
        ...
        var totalLevel = parseInt(v["spellclass-" + spellclassidx + "-level-total"], 10) || 0;
        var spellClassExists = parseInt(v["spellclass-" + spellclassidx + "-exists"], 10) || 0;
        var usesSpells = parseInt(v.use_spells, 10) || 0;
        var casterlevel = 0;
        var setter = {};
        var recalcAfter = 0;
        var params = {};
...
function setCasterClassFromDropdown(spellclassidx, eventInfo, callback, silently) {
    ...
    getAttrs([spellclassdropdown, spellclasslevel], function(va) {
        ...
        var currClassLevel = parseInt(va[spellclasslevel], 10);
        var spellclassname;
        var classname;
        var classlevel;
        try {
            ...
            getAttrs([classname, classlevel, spellclassname], function(v) {
                var setter = {};
                var updateLevel = 0;
                var newClassLevel = parseInt(v[classlevel], 10) || 0;
                try {
...
export function updateCasterFromClassLevel(classidx, eventInfo, force, callback, silently) {
    var spellclassdropdown0 = "spellclass-0";
    var spellclassdropdown1 = "spellclass-1";
    var spellclassdropdown2 = "spellclass-2";
    ...
    getAttrs([spellclassdropdown0, spellclassdropdown1, spellclassdropdown2], function(va) {
        ...
        getAttrs([spellclasslevelField, classlevelField, classNameField], function(v) {
            var setter = {};
            var newCasterLevel = parseInt(v[classlevelField], 10) || 0;
            var currCasterLevel = parseInt(v[spellclasslevelField], 10);
...
export function recalculate(callback, silently, oldversion) {
    ...
    function recalcTopSection(callback, silently) {
        var doneOne = _.after(3, function() {
...
function registerEventHandlers() {
    ...
    _.each(PFConst.spellClassIndexes, function(spellClassIdx) {
        ...
        _.each(PFSpells.spellLevels, function(spellLevel) {
            ...
            var eventToWatch = events.spellcastingClassEventsPerSpellLevel.replace(/CLASSIDX/g, numberIdx).replace(/SPELLLEVEL/g, spellNumber);
