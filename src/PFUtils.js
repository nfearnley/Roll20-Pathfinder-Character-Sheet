...
export function findSpellRange(customRangeVal, rangeDropdown, casterlevel) {
    var newRange = 0;
    var ranges = calculateSpellRanges(casterlevel);
    casterlevel = casterlevel || 0;
...
export function getAutoHPPercentMultiplier(autohp_percent) {
    ...
    case 1: newhealth = 0.5;
        break;
    case 2: newhealth = 0.75;
        break;
    case 3: newhealth = 1;
        break;
    default: newhealth = 0.5;
        break;
    }
...
export function getDiceDieFromString(str, cleanedup, atStart) {
    var matches;
    var ret = { "dice": 0, "die": 0, "plus": 0, "spaces": 0 };
    var sign = 1;
    if (!str) {
        return ret;
    }
    ...
    matches = str.match(PFConst.diceDiereg);
    if (matches) {
...
/**
 * Puts inline roll brackets [[ ]] around 'xdy +z' dice strings (z exists or not)
...
export function getCompendiumFunctionSet(prefix, field, methodToCall, v, setter, setField) {
    var temp = 0;
    var attr = v[prefix + field + "_compendium"];
    setter = setter || {};
